...
module.exports = function (grunt) {
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new botvac name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/botvac.png')) {
            fs.renameSync(__dirname + '/admin/botvac.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/botvac.html')) {
            fs.renameSync(__dirname + '/widgets/botvac.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/botvac/js/botvac.js')) {
            fs.renameSync(__dirname + '/widgets/botvac/js/botvac.js', __dirname + '/widgets/botvac/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/botvac')) {
            fs.renameSync(__dirname + '/widgets/botvac',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Botvac/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Botvac'
                        },
                        {
                            match: /Pmant/g,
                            replacement: author
