        ...
        if (channel === 'commands') {
            switch (command) {
                ...
                case 'pause':
                    if (!state.val) {
                        adapter.setState(id, false, true);
                        adapter.log.warn(
                            'use resume or stop state if you want to resume or stop cleaning ' + robotName,
                        );
                        return;
                    }
                    updateRobot(allRobots[robotName], function (error) {
                ...
                case 'stop':
                    if (!state.val) {
                        adapter.setState(id, false, true);
                        adapter.log.warn(
                            'use start or resume state if you want to start or resume cleaning ' + robotName,
                        );
                        return;
                    }
                    updateRobot(allRobots[robotName], function (error) {
            ...
            if ((match = String(command).match(reStartTime)) !== null && match.length === 2) {
                ...
                if (!(reTimeValidator.test(state.val) || state.val === '')) {
                    adapter.log.warn(
                        'Not valid format of time for ' + JSON.stringify(command) + ' = ' + JSON.stringify(state.val),
                    );
                ...
                allRobots[robotName].getSchedule(true, function (error, schedule) {
                    if (error || !schedule) {
                        adapter.log.warn('could not update robot ' + robotName);
                        adapter.setState(robotName + '.status.reachable', true, false);
                        return;
                    }
                    ...
                    if (Object.prototype.hasOwnProperty.call(schedule, 'events')) {
                        ...
                        if (isDay >= 0) {
                            ...
                            if (allRobots[robotName].availableServices.schedule !== 'minimal-1') {
                                adapter.getState(
                                    robotName + '.' + channel + '.' + day + '-mode',
                                    function (err, modeState) {
                                        if (err) {
                                            adapter.log.warn(
                                                'cannot get mode for day ' +
                                                    day +
                                                    'for ' +
                                                    robotName +
                                                    '! Error: ' +
                                        ...
                                        adapter.log.debug(
                                            'mode: ' + JSON.stringify(modeState) + ' for day ' + JSON.stringify(day),
                                        );
                                        newDay['mode'] = modeState.val;
                                        if (allRobots[robotName].availableServices.schedule === 'basic-2') {
                                            adapter.getState(
                                                robotName + '.' + channel + '.' + day + '-boundaryId',
                                                function (err, boundaryState) {
                                                    if (err) {
                                                        adapter.log.warn(
                                                            'cannot get boundaryId for day ' +
                                                                day +
                                                                'for ' +
                                                                robotName +
                                                                '! Error: ' +
                                                    ...
                                                    newDay['boundaryId'] = boundaryState.val;
                                                    // add new day
                            ...
                            adapter.log.warn(
                                'Can not clear already empty schedule' +
                                    JSON.stringify(command) +
                                    ' = ' +
                                    JSON.stringify(state.val),
                            );
    ...
    client.authorize(mail, password, false, function (error) {
        ...
        client.getRobots(function (error, robots) {
            if (error || !robots.length) {
                adapter.log.warn(
                    'no robots found! Err = ' + JSON.stringify(error) + ' robots = ' + JSON.stringify(robots),
                );
                restart(300000);
                return;
            }
            prepareRobotsStructure(robots, {}, function (error, devices) {
                ...
                createRobotsObjects(devices, null, null, function (error, _result) {
                    if (error) {
...
function updateRobot(robot, callback) {
    ...
    robot.getState(function (error, state) {
        ...
        var lastCleaning =
            state.cleaning.category === 1
                ? 'manual'
                : state.cleaning.category === 2
                  ? 'house'
                  : state.cleaning.category === 4
                    ? 'house with nogo'
                    : 'spot';
...
function prepareRobotsStructure(robots, devices, callback) {
    if (!robots.length) {
        return typeof callback === 'function' ? callback(null, devices) : null;
    }
            ...
            if (error || !state) {
                ...
                if (
                    ...
                    for (var day = 0; day < 7; day++) {
                        ...
                        if (
                            state.availableServices.schedule === 'basic-1' ||
                            state.availableServices.schedule === 'basic-2'
                        ) {
...
function createRobotsObjects(devices, channels, states, callback) {
    if (states !== null) {
                            ...
                            if (keys.length) {
                                ...
                                if (keys.length) {
                                    ...
                                    if (typeof callback === 'function') {
                                        ...
                                        adapter.log.warn(
                                            'createRobotsObjects(): callback not a function: ' +
        ...
        if (keys.length) {
            ...
            if (typeof callback === 'function') {
                callback(
                    'something wrong with channels : ' +
                        JSON.stringify(channels) +
                        ' for device: ' +
                        JSON.stringify(Object.keys(devices)[0]),
                    false,
                );
            } else {
